...
std::vector<SalesRecord> SqlSalesRecordRepository::getAll() {
    ...
    if (!dbConnector->ensureConnected()) return salesRecord;
    ...
    std::string query = "SELECT NameSong, Artist, Genre, Sold, Revenue FROM sales_record";
...
bool SqlSalesRecordRepository::add(const SalesRecord& record) {
    ...
    std::string artist = record.getArtist();
    std::string genre = record.getGenre();
    ...
    int duplicate = 0;
    if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS) {
        std::string checkQuery = "SELECT COUNT(*) FROM sales_record WHERE NameSong = ? AND Artist = ?";
    ...
    if (duplicate == 1) {
        ...
        if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS) {
            std::string selectQuery = "SELECT Sold, Revenue FROM sales_record WHERE NameSong = ? AND Artist = ?";
        ...
        if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS) {
            std::string updateQuery = "UPDATE sales_record SET Sold = ?, Revenue = ? WHERE NameSong = ? AND Artist = ?";
        ...
        int newId = 1;
        if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS) {
            std::string idQuery = "SELECT ISNULL(MAX(ID), 0) + 1 FROM sales_record";
        ...
        if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS) {
            std::string insertQuery = "INSERT INTO sales_record (ID, NameSong, Artist, Genre, Sold, Revenue) "
                                      "VALUES (?, ?, ?, ?, ?, ?)";
...
SalesRecord SqlSalesRecordRepository::getById(int id) {
    ...
    if (!dbConnector->ensureConnected()) return record;
    ...
    std::string query = "SELECT NameSong, Artist, Genre, Sold, Revenue FROM sales_record WHERE ID = ?";
